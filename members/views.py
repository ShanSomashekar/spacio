    ...
    if 'user' in request.session:
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                #     WHERE MemberID = %s
                # """, [member_id])
                # invoices = cursor.fetchall()  # List of tuples
...
def book_space(request):
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute("""
                    INSERT INTO Bookings (MemberID, SpaceID, StartDate, EndDate, Status)
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT s.SpaceID, s.Name, p.hourly_price,p.monthly_price, p.yearly_price, s.OccupationStatus
            FROM Spaces s
            JOIN price p ON s.SpaceID = p.SpaceID
            WHERE s.OccupationStatus = 'Available'
        """)
